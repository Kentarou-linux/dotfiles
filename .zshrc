...
if [ "$color_prompt" = yes ]; then
    PROMPT=$'%F{%(#.yellow.yellow)}┌──${debian_chroot:+($debian_chroot)──}(%B%F{%(#.red.cyan)}%n%(#.💀.㉿)%m%b%F{%(#.blue.yellow)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.yellow)}]\n└─%B%(#.%F{red}#.%F{red}$)%b%F{reset} '
    ...
    PROMPT='${debian_chroot:+($debian_chroot)}%n@%m:%%F{red}%~%f# '
fi
